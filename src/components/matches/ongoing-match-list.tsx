...
interface OngoingMatchListProps {
  matches: Match[];
}

export function OngoingMatchList({ matches }: OngoingMatchListProps) {
  return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold font-headline mb-4">Ongoing Matches</h2>
      {matches && matches.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         ...
         <div className="text-center py-10 border-2 border-dashed rounded-lg">
          <h3 className="text-lg font-semibold text-muted-foreground">No matches are currently in progress.</h3>
        </div>
      )}
    </div>
  );
