...
interface ActiveMatchListProps {
  matches: Match[];
}

export function ActiveMatchList({ matches }: ActiveMatchListProps) {
  return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold font-headline mb-4">Active Matches</h2>
      {matches && matches.length > 0 ? (
        ...
        <div className="text-center py-10 border-2 border-dashed rounded-lg">
          <h3 className="text-lg font-semibold text-muted-foreground">You haven't joined or created any matches.</h3>
          <p className="text-muted-foreground mt-2">Find a match below or create your own!</p>
        </div>
      )}
    </div>
  );
