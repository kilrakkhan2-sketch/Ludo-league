...
const legalNavItems = [
    { href: '/terms-and-conditions', icon: Gavel, label: 'Terms & Conditions' },
    { href: '/privacy-policy', icon: Shield, label: 'Privacy Policy' },
    { href: '/refund-policy', icon: Landmark, label: 'Refund Policy' },
    { href: '/gst-policy', icon: FileText, label: 'GST Policy' },
];
    ...
    <div className="flex h-full max-h-screen flex-col gap-2 bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <Separator className="my-4" />
         <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
          <p className="px-3 py-2 text-xs font-semibold text-muted-foreground/80">Legal</p>
          {legalNavItems.map((item) => <NavLink key={item.href} {...item} />)}
        </nav>
      </div>
